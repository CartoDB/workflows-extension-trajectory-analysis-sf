...
def main(
    ...
    if not any(input_metrics):
        raise ValueError("Select at least one metric to compute")
    ...
    col_names = list(
        pd.json_normalize(df["properties"].apply(lambda x: json.loads(x))).columns
    )
    ...
    if df.empty or df.t.nunique() <= 1:
        ...
        if input_distance_bool and input_distance_column != "undefined":
            result[input_distance_column] = (
                None  # Use None instead of np.nan for JSON serialization
            )
    ...
    if input_distance_bool and input_distance_column != "undefined":
        traj.add_distance(
            ...
            units=distance_units[input_distance_unit_distance],
        )
        added_columns.append(input_distance_column)
    if input_duration_bool and input_duration_column != "undefined":
    ...
    if input_speed_bool and input_speed_column != "undefined":
        traj.add_speed(
            ...
            units=(
                distance_units[input_speed_unit_distance],
                time_units[input_speed_unit_time],
            ),
        )
        added_columns.append(input_speed_column)
    if input_acceleration_bool and input_acceleration_column != "undefined":
        traj.add_acceleration(
            ...
            units=(
                distance_units[input_acceleration_unit_distance],
                time_units[input_acceleration_unit_time],
                time_units[input_acceleration_unit_time],
            ),
        )
        added_columns.append(input_acceleration_column)
    ...
    if input_duration_bool:
        result[input_duration_column] = result[input_duration_column].apply(
            lambda x: x.total_seconds() / time_conversions[input_duration_unit_time]
            if pd.notna(x)
            else 0
        )
